...
public class ExtractParetoMol {
  ...
  public static void main(String[] args) throws IOException {
    ...
    String[] listMol = folder.list();
    
    for (int i = 0; i<listMol.length; i++){
      inputFileName = inputFilePath + listMol[i];
      if (Files.isRegularFile(Paths.get(inputFileName))){
        if (inputFileName.contains("FUN")){
