...
public class MultiPharmMRF extends AbstractDoubleProblem {
    ...
    public void evaluate(DoubleSolution solution) {
        ...
        double electrostatic = 0;
            ...
            coords = this.rotate1Axis(this.coordsTarget, solution.getVariableValue(0), solution.getVariableValue(1), solution.getVariableValue(2));
    
    
            // DISPLACEMENT
            coords = this.displacement(coords, solution.getVariableValue(3), solution.getVariableValue(4),
                    solution.getVariableValue(5));
            // Asign new coords
    ...
    private double[] rotate1Axis(double[] coords, double theta, double x2, double y2) {

        double[] new_coords = coords.clone();
        double vectorX = Math.cos(x2)*Math.sin(y2);
        double vectorY = Math.sin(x2)*Math.sin(y2);
        double vectorZ = Math.cos(y2); 
        ...
        q1w = cosTheta;
        q1x = sinTheta * vectorX;
        q1y = sinTheta * vectorY;
        q1z = sinTheta * vectorZ;
        ...
        for (int i = 0; i < (new_coords.length / 3); i++) {
            ...
            atomPositionMinusAX = coords[i3];
            atomPositionMinusAY = coords[i3 + 1];
            atomPositionMinusAZ = coords[i3 + 2];
            ...
            new_coords[i3] = part3x;
            new_coords[i3 + 1] = part3y;
            new_coords[i3 + 2] = part3z;
        }
        return new_coords;
    }
